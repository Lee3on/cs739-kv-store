...
func (kv *KV) FlushState() {
	kv.mu.Lock()
	defer kv.mu.Unlock()

	// Persist the serialized state in storage
	if err := kv.storage.SaveState(kv.kv); err != nil {
		log.Fatalf("[ERROR] %d failed to flush state to storage: %v", kv.id, err)
	}

	log.Printf("[INFO] %d state flushed to storage", kv.id)
}
