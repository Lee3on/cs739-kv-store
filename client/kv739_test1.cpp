    ...
    auto get_keys = [&]() {
        {
            ...
            cv.wait(lock, [&] { return is_first_put_done; });
        }
...
void test_put_get_order_2() {
    ...
    auto put_keys = [&]() {
        ...
        {
            std::unique_lock<std::mutex> lock(mtx);
            cv.wait(lock, [&] { return is_get_attempt_done; });
        }
    ...
    auto get_keys = [&]() {
        {
            std::unique_lock<std::mutex> lock(mtx);
            cv.wait(lock, [&] { return is_second_put_done; });
        }
        ...
        {
            std::unique_lock<std::mutex> lock(mtx);
            cv.wait(lock, [&] { return is_all_puts_done; });
        }
